...
/**
 * Search result structure from docs API
 */
...
export async function searchDocumentation(
  ...
  try {
    ...
    const searchResults = await handleSearchTool(query, {
      limit,
      topics,
    });

    const results = searchResults['results'] as unknown;
            ...
            text: JSON.stringify(
              {
                ...
                results: [],
                message:
                  'No documentation found matching your query. Try different keywords or check available topics.',
              },
              null,
              2
            ),
    ...
    const formattedResults = results.map((item: unknown) => {
      const result = item as DocSearchResult;
      return {
 ...
 */
function generateSearchSuggestions(_query: string, results: FormattedSearchResult[]): string[] {
  ...
  if (results.length === 0) {
    ...
    if (results[0]) {
      suggestions.push(
        `Top result: "${results[0].title}" - ${results[0].description || 'No description'}`
      );
    }
  }
